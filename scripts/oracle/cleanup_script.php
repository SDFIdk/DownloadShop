...
$result_pre = db_query('SELECT bundle.entity_id as userid, bundle.field_access_bundles_nid, predefined.entity_id, predefined.field_predefined_datacollections_target_id as serviceid FROM {field_data_field_access_bundles} as bundle
  ...
  WHERE predefined.bundle =:bundle_type', array(':bundle_type' => 'access_bundle'));

foreach ($result_pre as $record) {
  $counter++;
  remove_record_predefined($record->userid, $record->serviceid);
}
...
*   Service identifier
*/
function remove_record_predefined($userid, $serviceid){
  db_query('DELETE FROM {field_data_field_predefined_datacollections}
    WHERE entity_id = :entity_id AND field_predefined_datacollections_target_id = :webservice_id', array(':entity_id' => $userid, ':webservice_id' => $serviceid));
  db_query('DELETE FROM {field_revision_field_predefined_datacollections}
    WHERE entity_id = :entity_id AND field_predefined_datacollections_target_id = :webservice_id', array(':entity_id' => $userid, ':webservice_id' => $serviceid));
}
